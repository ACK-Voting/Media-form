...
export default function AboutPage() {
  const { leaders } = useLeadershipStore();
  const clergy = leaders.filter(l => l.ordained);
  const layLeaders = leaders.filter(l => !l.ordained);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Leadership */}
      <section id="clergy" className="py-20 bg-gradient-to-br from-gray-50 to-blue-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="inline-block bg-blue-100 text-blue-900 px-4 py-2 rounded-full text-sm font-semibold mb-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12 items-stretch">
            {clergy.map(leader => (
              <div key={leader.id} className="flex flex-col bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all border border-gray-100">
                {/* Photo */}
                <div className="h-64 bg-gradient-to-br from-blue-700 to-indigo-700 overflow-hidden flex-shrink-0">
                  {leader.photo ? (
                    <img src={leader.photo} alt={leader.name} className="w-full h-full object-cover [object-position:center_25%]" />
                  ) : (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
            {layLeaders.map(leader => (
              <div key={leader.id} className="flex flex-col bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl transition-all">
                <div className="h-64 bg-gradient-to-br from-blue-700 to-indigo-700 overflow-hidden flex-shrink-0">
                  {leader.photo ? (
                    <img src={leader.photo} alt={leader.name} className="w-full h-full object-cover [object-position:center_25%]" />
                  ) : (
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-blue-200 leading-relaxed mb-4">
                ACK Mombasa Memorial Cathedral belongs to the Anglican Church of Kenya, which is in the Anglican Communion —
